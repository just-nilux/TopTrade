...
## Concluson
Overall I am happy with the results of this project. Forecasting the price of an asset is an extremely difficult task with no simple solution. I was able to build a model that is correctly able to predict whether a cryptocurrency is a buy or a sell over a three interval trading period nearly 58 percent of the time. Although this is not an extremely high accuracy score, within the context of this specific problem (trading) only a slight edge against the market is needed to generate a profitable trading strategy. 
